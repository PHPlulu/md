<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:81:"D:\phpstudy\PHPTutorial\WWW\md\public/../application/index\view\tomb\glprice.html";i:1530774885;s:75:"D:\phpstudy\PHPTutorial\WWW\md\public/../application/index\view\layout.html";i:1533793060;}*/ ?>
<!DOCTYPE html>
<html>
	<head>
		<meta charset="utf-8" />
    ...
    <table class="table table-bordered">
      ...
      <?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
        <tr>
          ...
          <td>
            <?php if(($vo['manage_time'] >= $ltime)): ?>
              已付管理费￥<?php echo $vo['manage_sum_money']; ?>  年限 <?php echo $vo['manage_year']; ?>(年)
            <?php else: ?> 
              已过期
            <?php endif; ?>
          </td>
          <td><?php echo date("Y-m-d",$vo['manage_time']); ?></td>
          <td><?php echo $vo['money']; ?></td>
          <td><?php echo $vo['name']; ?></td>
          <td><?php echo $sysyst[$vo['relationship']]['title']; ?></td>
          <td>
            <?php if($vo['sex'] == 2): ?>
              男
            <?php else: ?> 
...
$('.show_all').click(function(){
    ...
    if(today){
      window.location.href="<?php echo url('/index/Tomb/glprice/gtime/"+gtime+"/today/"+today+"'); ?>";
    }else{
      window.location.href="<?php echo url('/index/Tomb/glprice/gtime/"+gtime+"'); ?>";  
    }
});
</script>
	             </div>
