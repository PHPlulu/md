<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:70:"E:\PHPTutorial\WWW\md\public/../application/index\view\tomb\buyed.html";i:1535102926;s:66:"E:\PHPTutorial\WWW\md\public/../application/index\view\layout.html";i:1533793060;}*/ ?>
<!DOCTYPE html>
<html>
	<head>
		<meta charset="utf-8" />
  ...
  $.ajax({
    ...
    <script type="text/javascript">
        ...
        function printmud(){
           ...
           $.ajax({
                ...
                success     : function(g){
                    ...
                    vhtml=layer.open({
                      type: 1,
                      title: '打印墓位证(订购信息)', //不显示标题
                      skin: 'layui-layer-rim', //加上边框
                      area: ['100%', '100%'], //宽高
                      content: g
                    });
                    $("#ele7").css("background-image","url(__IMG__/mu_bg.png)") ;
                    $("#ele7").css({"width":"763px","height":"582px",'margin':'0 auto'}) ;
                }
            });
        }
        function show_print_mud(){
            $("#ele7").print();
        }
        function printmul(){
            var dyzuser=$('input:radio[name="dyzuser"]:checked').val(); 
          if(dyzuser){
           ...
           $.ajax({
                ...
                success     : function(g){
                    ...
                    vhtml=layer.open({
                      type: 1,
                      title: '打印墓位证(落葬信息)', //不显示标题
                      skin: 'layui-layer-rim', //加上边框
                      area: ['100%', '100%'], //宽高
                      content: g
                    });
                    $("#ele8").css("background-image","url(__IMG__/mu_bg.png)") ;
                    $("#ele8").css({"width":"763px","height":"582px",'margin':'0 auto'}) ;
                }
            });
          }else{
              layer.msg('无墓位信息或没有选择');
        ...
        function closexujiao(){
          var ssxdc=layer.confirm('您确定要取消操作吗？', {
            btn: ['确定','取消'] //按钮
          }, function(){
            layer.close(ssxdc);
         ...
         function glmone(){
          ...
          function setxujiao(){
            ...
            var sssszax=layer.confirm('请核实信息无误，点击确定？', {
              btn: ['确定','取消'] //按钮
            }, function(){
              layer.close(sssszax);
          ...
          function laclogzdj(){
            var ssxc=layer.confirm('您确定要取消操作吗？', {
              btn: ['确定','取消'] //按钮
            }, function(){
              layer.close(xujiaohtml);
              layer.close(ssxc);
        ...
        function setcardimgclose(){
          ...
          function setgzdj(eid){
            ...
            var sscds=layer.confirm('请核实信息无误，点击确定？', {
              btn: ['确定','取消'] //按钮
            }, function(){
              layer.close(sscds);
              $.ajax({
                  ...
                  success     : function(g){
                      if(g == 'ok'){
                          ...
                          $(".gztan input").attr("disabled",true);
                          $(".gztan input").css("backgroung","rgb(198, 198, 198)");
                          ...
                          $("#relationship").css("backgroung","rgb(198, 198, 198)");
                          $("#sex").css("backgroung","rgb(198, 198, 198)");
                          $("#dead_sex").attr("disabled",true);
                          $("#dead_sex").css("backgroung","rgb(198, 198, 198)");
                          $("#cstype").attr("disabled",true);
                          $("#cstype").css("backgroung","rgb(198, 198, 198)");  
                          $("#lztype").attr("disabled",true);
                          $("#lztype").css("backgroung","rgb(198, 198, 198)");
                          $("#sstype").attr("disabled",true);
                          $("#sstype").css("backgroung","rgb(198, 198, 198)");
                          layer.msg('设置成功');
                      }else if(g == 'no'){
                          layer.close(htmlgzdj);
                          layer.close(xujiaohtml);
...
//                    success     : function(g){
//                        if(g == 'ok'){
//                            $('.bwtantable table tbody').append('<tr class="row_lzxxarr"><td><input type="radio" name="zanguser" value="12"><a href="javascript:;">修改</a></td><td>111111</td><td>1</td><td>11111111111</td><td>2018-08-17</td><td>2018-08-24</td><td>2018-08-24</td><td>2018-08-24</td><td>2018-08-24</td><td>1</td><td>1</td><td>1</td></tr>');
//                            layer.close(htmlbcs);
//                            layer.msg('设置成功');
//                        }else if(g == 'no'){
//                            layer.close(htmlbcs);
...
<script type="text/javascript">
    ...
    // 编辑故者信息
    function ding_update(id){
      imghtml = layer.load(0, {
        shade: [0.5] //0.1透明度的白色背景
      });
      $.ajax({
          type: 'post',
          url: "<?php echo url('Tomb/news_update'); ?>",
          dataType: 'json',
          data: {
              type:21,
              id : id,
