...
class Sysjcw extends Root {
    ...
    static public function dep_sell_all ($arra){
        $map = [];
        if ($arra['cem_id'] && $arra['cem_id'] !='0') {
            $map['sysid'] = $arra['cem_id'];
        }
        if ($arra['cem_area_id'] && $arra['cem_area_id'] !='0') {
            $map['sysid_s'] = $arra['cem_area_id'];
        }
        if ($arra['cem_row_id'] && $arra['cem_row_id'] !='0') {
            $map['sysid_c'] = $arra['cem_row_id'];
        }
        if ($arra['sysysid']) {
            $map['sysysid'] = $arra['sysysid'];
        }
        if ($arra['syszt']) {
            $map['syszt'] = $arra['syszt'];
        }
        $arr=self::where($map)->select();
            $html='';
            foreach ($arr as $key => $v) {
                $html.='<div class="wlistall">';
                $html.='<div class="dinga"  class="tr_'.$v['id'].'" row_id = "'.$v['id'].'" >';
                  $html.='<a href="javascript:open('.$v['id'].');">';
                    $html.='<div class="dingimg">';
                      if($v['syszt'] == 1){
                        $html.='<img src="/index/images/kong.png" style="width:42px;height:42px;">';
                      }else if($v['syszt'] ==2){
                        $html.='<img src="/index/images/ding_03.png" style="width:42px;height:42px;">';
                      }else if($v['syszt'] == 3){
                        $html.='<img src="/index/images/zang_03.png" style="width:42px;height:42px;">';
                      }
                    $html.='</div>';
                    $html.='<p>'.$v['long_title'].'</p>';
                  $html.='</a>';
                $html.='</div>';
                $html.='</div>';
            }
